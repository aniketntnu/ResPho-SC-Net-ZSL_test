...
import os
os.environ["TORCH_HOME"] = "./torch_cache"  # set BEFORE importing torchvision

from modules import  residualmodels,models
from modules import  residualmodels_1 
from modules import ResNet51PretrainOctopus_
...
def main(args):
    ...
    logger.info(f'Dataset created with {len(dataset_train)} training samples.')
    print(f'Dataset created with {len(dataset_train)} training samples.')
    logger.info(f'Dataset created with {len(dataset_valid)} validation samples.')
    print(f'Dataset created with {len(dataset_valid)} validation samples.')
    ...
    if args.mode == 'train' or args.mode == 'test':
        ...
        data_loader_test_seen = DataLoader(
            dataset_test_seen,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            shuffle=True,
        ...
        data_loader_test_unseen = DataLoader(
            dataset_test_unseen,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            shuffle=True,
    ...
    def training():
        ...
        if 0:#os.path.isfile(args.name+"//epoch.pt"):
            ...
            try:
                ...
                print("\n\t exception in loading pretrained weights:",e)
                new_state_dict = torch.load(args.pretrained_weights, map_location="cpu")
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if 1:#(epoch%10)-1 == 0 and epoch>1:

                print("\n\t calling test")
                logger.info(f'Calling testing at epoch {epoch}')
                testing(epoch)
            ...
            print(f'Epoch {epoch} training complete. Mean loss: {mean_loss:.4f}')
    ...
    def testing(epoch = 1000):
        ...
        if args.testing_mode == 'zsl' or args.testing_mode == 'gzsl':
            ...
            #acc_seen, _, __ = zslAccuracyTest(model, data_loader_test_seen, device,promptModel,0)
            #acc_unseen, _, __ = zslAccuracyTest(model, data_loader_test_unseen, device,promptModel,0)
        ...
        if args.testing_mode == 'zsl' or args.testing_mode == 'gzsl':
            if 0:#args.words_list is not None:
        ...
        with open(args.name + '/' + 'testresults.txt', 'a') as f:
            ...
            f.write(f'Seen acc: {acc_seen1}\n')
            f.write(f'Unseen acc: {acc_unseen1}\n')
